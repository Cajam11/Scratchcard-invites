...
export default function ScratchCard({
  ...
  // Set dimensions dynamically based on container content and viewport
  const [dimensions, setDimensions] = useState({ width: 680, height: 280 });

  useEffect(() => {
    if (containerRef.current) {
      const { width, height } = containerRef.current.getBoundingClientRect();
      // Use responsive minimums based on viewport width
      const minWidth = Math.max(width, window.innerWidth < 640 ? 280 : 420);
      const minHeight = Math.max(height, window.innerWidth < 640 ? 140 : 220);
  ...
  const scratch = (clientX: number, clientY: number) => {
    ...
    ctx.beginPath();
    // Scale scratch radius based on container size for consistent UX
    const scratchRadius = Math.max(20, dimensions.width * 0.04);
    ctx.arc(
      clientX - rect.left,
      clientY - rect.top,
      scratchRadius,
  ...
  return (
    <div
      className={`relative group mx-auto w-full max-w-3xl overflow-hidden rounded-2xl border border-[rgba(53,88,140,0.55)] flex flex-col ${containerClassName ?? ""}`}
      ref={containerRef}
    >
      {/* Background content (The real details to be revealed) */}
      <div
        className={`min-h-[240px] flex-1 bg-[rgba(7,10,20,0.92)] px-6 py-8 sm:px-10 flex items-center justify-center pointer-events-none w-full ${contentClassName ?? ""}`}
      >
        {children}
      </div>
      ...
      {!revealed && (
        <canvas
          ...
          onPointerMove={(event) =>
            event.buttons === 1 && scratch(event.clientX, event.clientY)
          }
        />
      )}
    </div>
